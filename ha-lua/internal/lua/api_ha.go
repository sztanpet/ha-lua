...
type haAPI struct {
	...
	keepIDs []string
	// pruned is set by the runner once PruneScript has consumed keepIDs. See
	// keepTimer: after that point keepIDs has no reader left, so registrations
	// from callbacks must stop appending to it.
	pruned bool

...
### Fixed
- **The debug page's routes and timers each get their own line.** Routes were
  comma-joined into one unreadable run; timers were already built with
  newlines, but the cell's styling collapsed them, so they ran together too.
...
### Added
- **Add-on icon.** The add-on no longer shows as a generic puzzle piece in
  the store and sidebar: a house cut out of a planet in HA brand blue, with
  a small moon on an orbit arc — a nod to Lua's planet-and-moon mark. The
  icon is generated by `tools/icon` from the Go standard library alone.

### Changed
- **Card 0.3.28: the "held until" badge sits on the title row.** It was a
  flex sibling of the whole two-line heading, so it floated vertically
  between the title and subtitle lines. It now sits at the right edge of
  the card, vertically centered on the title line.

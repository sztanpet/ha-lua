...
boot: auto
map:
  # HA config dir mounted at /config (homeassistant_config + path override),
  # so scripts live at config/ha-lua/scripts next to configuration.yaml —
  # like the ESPHome add-on. addon_config would hide them in a private folder.
...
- **Scripts**: live at `/config/ha-lua/scripts/` — the HA config directory is mounted at `/config`, so this is the same path on the host (`config/ha-lua/scripts/`, next to `configuration.yaml`), editable via Studio Code Server.
...
No flags. In add-on mode (no `--config` flag) the binary reads `/data/options.json` for user options, takes the token from `$SUPERVISOR_TOKEN`, and hardcodes the rest: URL `ws://supervisor/core/websocket`, scripts at `/config/ha-lua/scripts`, DB at `/data/ha-lua.db`. There is exactly one source for each value — flags duplicating what the environment and options.json already provide is how the two drift apart.
